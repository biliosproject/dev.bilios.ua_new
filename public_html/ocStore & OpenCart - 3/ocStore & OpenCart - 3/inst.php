...
$stmt = $dbh->prepare("INSERT INTO ".$pr."pars SET id=1, hash='', key_lic='free', mod_ver='1'");
$stmt->execute();
...
$stmt = $dbh->prepare("CREATE TABLE `".$pr."pars_xml` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `dn_id` int(11) NOT NULL,
  `cat_work` int(1) NOT NULL DEFAULT '0',
  `pr_start` varchar(256) NOT NULL,
  `pr_stop` varchar(256) NOT NULL,
  `cat_start` varchar(256) NOT NULL,
  `cat_stop` varchar(256) NOT NULL,
  `cat_name_start` varchar(256) NOT NULL,
  `cat_name_stop` varchar(256) NOT NULL,
  `cat_id_start` varchar(256) NOT NULL,
  `cat_id_stop` varchar(256) NOT NULL,
  `cat_perent_start` varchar(256) NOT NULL,
  `cat_perent_stop` varchar(256) NOT NULL,
  `pr_cat_start` varchar(256) NOT NULL,
  `pr_cat_stop` varchar(256) NOT NULL,
  `cat_delim` varchar(256) NOT NULL DEFAULT '{csvnc}',
  PRIMARY KEY (`id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_setting` ADD `r_layout_pr` INT NOT NULL DEFAULT '0' AFTER `r_status`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_prsetup` ADD `layout_pr` VARCHAR(5) NOT NULL DEFAULT '0' AFTER `status`, ADD `layout_cat` VARCHAR(5) NOT NULL DEFAULT '0' AFTER `layout_pr`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_setting` ADD `r_tags` TEXT NOT NULL AFTER `r_layout_pr`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_prsetup` ADD `tags` TEXT NOT NULL AFTER `layout_cat`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_setting` ADD `r_des_dir` INT(1) NOT NULL DEFAULT '0' AFTER `r_des`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_prsetup` ADD `des_dir` VARCHAR(100) NOT NULL DEFAULT 'description' AFTER `des_d`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_prsetup` ADD `img_name` VARCHAR(256) NOT NULL AFTER `img_dir`");
$stmt->execute();


$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_setting` ADD `filter_round_rules` TEXT NOT NULL AFTER `filter_round_domain`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_setting` ADD `filter_link_rules` TEXT NOT NULL AFTER `filter_link_domain`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_setting` ADD `u_manufac` INT(2) NOT NULL DEFAULT '0' AFTER `grans_permit`, ADD `u_des` INT(2) NOT NULL DEFAULT '0' AFTER `u_manufac`, ADD `u_cat` INT(2) NOT NULL DEFAULT '0' AFTER `u_des`, ADD `u_img` INT(2) NOT NULL DEFAULT '0' AFTER `u_cat`, ADD `u_attr` INT(2) NOT NULL DEFAULT '0' AFTER `u_img`, ADD `u_opt` INT(2) NOT NULL DEFAULT '0' AFTER `u_attr`, ADD `u_made_meta` INT(2) NOT NULL DEFAULT '0' AFTER `u_opt`, ADD `u_up_url` INT(2) NOT NULL DEFAULT '0' AFTER `u_made_meta`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_browser` ADD `cookie_up` INT(2) NOT NULL DEFAULT '0' AFTER `cookie_use`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_browser` ADD `auth_url` TEXT NOT NULL AFTER `ch_pattern`, ADD `auth_data` TEXT NOT NULL AFTER `auth_url`, ADD `auth_type` INT(1) NOT NULL DEFAULT '1' AFTER `auth_data`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_browser` ADD `auth_url_check` TEXT NOT NULL AFTER `auth_type`, ADD `auth_str` VARCHAR(256) NOT NULL AFTER `auth_url_check`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_prsetup` ADD `opt_imgs` VARCHAR(2000) NOT NULL AFTER `opt_quant_d`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."product` ADD `cost` DECIMAL(15,4) NOT NULL AFTER `price`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_setting` ADD `r_cost` INT(1) NOT NULL DEFAULT '0' AFTER `r_price_spec_date_end`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_prsetup` ADD `cost` TEXT NOT NULL AFTER `price_spec`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_browser` CHANGE `auth_url` `auth_url` VARCHAR(256) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_browser` CHANGE `auth_url_check` `auth_url_check` VARCHAR(256) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_browser` ADD `auth_use` INT NOT NULL DEFAULT '0' AFTER `ch_pattern`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_browser` ADD `webp_conv` INT NOT NULL DEFAULT '0' AFTER `auth_str`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_prsetup` ADD `hpm_sku` VARCHAR(10) NOT NULL AFTER `manuf_seo_keyw`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_setting` ADD `r_hpm` INT(1) NOT NULL DEFAULT '0' AFTER `r_tags`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_setting` ADD `scripts_permit` INT(1) NOT NULL DEFAULT '0' AFTER `grans_permit`");
$stmt->execute();

$stmt = $dbh->prepare("CREATE TABLE `".$pr."pars_phpscripts` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `dn_id` int(11) NOT NULL DEFAULT '0',
  `status` int(1) NOT NULL DEFAULT '0',
  `name` varchar(256) NOT NULL,
  `when_do` int(2) NOT NULL DEFAULT '0',
...
$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_prsetup` ADD `related_sku` VARCHAR(10) NOT NULL AFTER `manuf_seo_keyw`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_setting` ADD `r_related` INT NOT NULL DEFAULT '0' AFTER `r_tags`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_setting` ADD `r_price_spec` INT NOT NULL DEFAULT '1' AFTER `r_price`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_xml` ADD `filter_yes` TEXT NOT NULL AFTER `cat_delim`, ADD `filter_no` TEXT NOT NULL AFTER `filter_yes`");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_link` CHANGE `link` `link` VARCHAR(360) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_sen_link` CHANGE `link` `link` VARCHAR(360) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_link` ADD INDEX `link` (`link`)");
$stmt->execute();

$stmt = $dbh->prepare("ALTER TABLE `".$pr."pars_sen_link` ADD INDEX `link` (`link`)");
$stmt->execute();
...
if ($php_v < 5.6) {
  ...
  php-7.2<br>
  php-7.3<br>
  Откройте директорию с вашей версией PHP и скопируйте содержимое в корень сайта с заменой.<br>
  <b>После выполнения вышеописанных шагов, просто закройте эту страницу</b>.';
}else{
