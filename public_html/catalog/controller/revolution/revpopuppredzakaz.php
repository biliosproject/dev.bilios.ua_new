...
class ControllerRevolutionRevpopuppredzakaz extends Controller {
	public function index() {
		...
		if ($product_info) {
			...
			if (!$settings['notify_status']) {
				...
				$data['heading_title'] = $this->language->get('text_mail_notify');
				$data['button_shopping'] = $this->language->get('button_shopping');
				$data['button_checkout'] = $this->language->get('entry_rev_p_send');
	...
	public function make_order_notify() {
		$json = array();
		...
		if (!isset($json['error'])) {
			$data = array(
				'firstname'    => (isset($this->request->request['firstname'])) ? $this->request->request['firstname'] : '',
				'telephone'    => (isset($this->request->request['telephone'])) ? $this->request->request['telephone'] : '',
				'email'        => (isset($this->request->request['email']) && !empty($this->request->request['email'])) ? $this->request->request['email'] : 'localhost@localhost.com',
			...
			$json['output'] = $this->language->get('text_success_notify');
		}
