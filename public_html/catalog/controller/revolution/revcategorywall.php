...
class ControllerRevolutionRevcategorywall extends Controller {
    public function index() {
		...
		$data['setting_all_settings'] = $setting_all_settings = $this->config->get('revtheme_all_settings');
		...
		$data['slider'] = false;
		if ($setting['group_manufs'] && $setting['group_manufs'] == 'slider') {
			$data['slider'] = true;
		}
		$data['chislo_ryad'] = $this->config->get('revtheme_catalog_all')['chislo_ryad'];
		...
		if ($cache_data) {
			...
			if ($setting['categories'] == 1 && count($revtheme_home_categorywall_categories)>0) {
				foreach ($revtheme_home_categorywall_categories as $category_id) {
					...
					if ($category_info) {
						if ($setting['group_manufs'] && $setting['group_manufs'] != 'wall' && $setting['group_manufs'] != 'slider') {
						...
						} else if ($setting['group_manufs'] && $setting['group_manufs'] == 'wall') {
							$children_data = array();
				...
				foreach ($revtheme_home_categorywall_manufacturers as $manufacturer_id) {
					...
					if ($category_info) {
						if ($setting['group_manufs'] && $setting['group_manufs'] == '1') {
							$name = $category_info['name'];
							if (is_numeric(utf8_substr($name, 0, 1))) {
								$key = '0 - 9';
							} else {
