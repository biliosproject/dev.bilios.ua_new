...
class ModelRevolutionRevolution extends Model {
	...
	public function getBestProducts($data) {
		...
		$cache = 'revpbest.' . crc32((int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . (int)$customer_group_id . md5(http_build_query($data)));
        $results = $this->cache->get($cache);
		if (!$results) {
			...
			foreach ($query->rows as $result) {
				$results[$result['product_id']] =  $result['product_id'];
			}

			$setting_all_settings = $this->config->get('revtheme_all_settings');
			if ($setting_all_settings['cache_on']) {
				$this->cache->set($cache, $results);
			}
		}
		return $results;
	}
	
	public function getBestProductsTotal($data) {
		if ($this->customer->isLogged()) {
          $customer_group_id = $this->customer->getGroupId();
        } else {
          $customer_group_id = $this->config->get('config_customer_group_id');
		...
		$cache = 'revpbest_total.' . crc32((int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . (int)$customer_group_id . md5(http_build_query($data)));
        $revpbest_total = $this->cache->get($cache);
		if (!$revpbest_total) {
			$settings = $this->config->get('revtheme_catalog_best');
			$product_total = $settings['limit'];
			$revpbest_total = array();
			...
			$query = $this->db->query($sql);
			if ($query->rows) {
				$revpbest_total = count($query->rows);
			} else {
				$revpbest_total = 0;
			}
			...
			$setting_all_settings = $this->config->get('revtheme_all_settings');
			if ($setting_all_settings['cache_on']) {
				$this->cache->set($cache, $revpbest_total);
			}
		}
		return $revpbest_total;
	}
	...
	public function getLastProducts($data) {
		...
		$cache = 'revplast.' . crc32((int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . (int)$customer_group_id  . md5(http_build_query($data)));
        $results = $this->cache->get($cache);
		if (!$results) {
	...
	public function getLastProductsTotal($data) {
		if ($this->customer->isLogged()) {
          $customer_group_id = $this->customer->getGroupId();
        } else {
          $customer_group_id = $this->config->get('config_customer_group_id');
		...
		$cache = 'revplast_total.' . crc32((int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . (int)$customer_group_id . md5(http_build_query($data)));
        $revplast_total = $this->cache->get($cache);
		if (!$revplast_total) {	
			$settings = $this->config->get('revtheme_catalog_last');
			$product_total = $settings['limit'];
			
			$revplast_total = array();
			...
			$query = $this->db->query($sql);
			if ($query->rows) {
				$revplast_total = count($query->rows);
			} else {
				$revplast_total = 0;
			}
			...
			$setting_all_settings = $this->config->get('revtheme_all_settings');
			if ($setting_all_settings['cache_on']) {
				$this->cache->set($cache, $revplast_total);
			}
		}
		return $revplast_total;
	}
	...
	public function SendMail($data) {
		...
		$message = $this->language->get('text_mail_firstname') . ' '  .  $data['firstname'] . "\n";
		$message .= $this->language->get('text_mail_telephone') . ' '  .  $data['telephone'] . "\n";
		$message .= $this->language->get('text_mail_email') . ' '  .  $data['email'] . "\n";
		$message .= $this->language->get('text_mail_comment') . ' '  .  $data['comment'] . "\n";
	...
	public function getproductTabs($product_id){
		$tabs_data = array();
		$query = $this->db->query("SELECT * FROM ".DB_PREFIX."tabs t LEFT JOIN ".DB_PREFIX."tabs_description td ON(t.tab_id = td.tab_id) LEFT JOIN " . DB_PREFIX . "tabs_to_store t2s ON (t.tab_id = t2s.tab_id) WHERE t.status = 1 AND td.language_id = '".(int)$this->config->get('config_language_id')."' AND t2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY t.sort_order ASC");
	...
	public function getproducttextblocks($product_id){
		...
		$query = $this->db->query("SELECT * FROM ".DB_PREFIX."textblocks t LEFT JOIN ".DB_PREFIX."textblocks_description td ON(t.tb_id = td.tb_id) WHERE t.status = 1 AND td.language_id = '".(int)$this->config->get('config_language_id')."' AND t.stores LIKE '%".(int)$this->config->get('config_store_id')."%' ORDER BY t.sort_order ASC");
		$manufacturer_info = $this->getmanufacturertabs($product_id);
		if($query->rows){
			foreach($query->rows as $row){
				$status = false;
	...
	public function ajaxLiveSearchCats($data=array()) {
		$sql = "SELECT cd.category_id, cd.name FROM " . DB_PREFIX . "category_description cd WHERE";
		$implode = array();
		$words = explode(' ', trim(preg_replace('/\s\s+/', ' ', $data['filter_name'])));
		foreach ($words as $word) {
			$implode[] = "LCASE(cd.name) LIKE '%" . $this->db->escape($word) . "%'";
		}
		if ($implode) {
		...
		foreach ($query->rows as $result) {
			$category_data[$result['category_id']] = $this->model_catalog_category->getCategory($result['category_id']);
		}
		return $category_data;
	}
	
	public function ajaxLiveSearchMans($data=array()) {
		$sql = "SELECT m.manufacturer_id, m.name FROM " . DB_PREFIX . "manufacturer m WHERE";
		$implode = array();
		$words = explode(' ', trim(preg_replace('/\s\s+/', ' ', $data['filter_name'])));
		foreach ($words as $word) {
			$implode[] = "LCASE(m.name) LIKE '%" . $this->db->escape($word) . "%'";
		}
		if ($implode) {
			$sql .= " " . implode(" AND ", $implode) . "";
		}
		$sql .= " GROUP BY m.manufacturer_id";
		$sql .= " ORDER BY LCASE(m.name) ASC";
		if (isset($data['start']) || isset($data['limit'])) {
			if ($data['start'] < 0) {
				$data['start'] = 0;
			}				
		...
		foreach ($query->rows as $result) {
			$category_data[$result['manufacturer_id']] = $this->model_catalog_manufacturer->getManufacturer($result['manufacturer_id']);
		}
		return $category_data;
	}
	...
	public function getBlogs($data = array()) {
		$cache_blogmod_data = md5(http_build_query($data));
		$blogmod_cache = $blog_data = $this->cache->get('blogmod_cache.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . (int)$this->config->get('config_customer_group_id') . '.' . $cache_blogmod_data);
		if (!$blogmod_cache) {
			...
			$setting_all_settings = $this->config->get('revtheme_all_settings');
			if ($setting_all_settings['cache_on']) {
				$this->cache->set('blogmod_cache.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . (int)$this->config->get('config_customer_group_id') . '.' . $cache_blogmod_data, $blog_data);
			}
		}
		return $blog_data;
	}
	...
	public function getBlogsRelated($product_id) {
		$blog_data = array();
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "revblog_product bp LEFT JOIN " . DB_PREFIX . "revblog b ON (bp.revblog_id = b.revblog_id) WHERE bp.product_id = '" . (int)$product_id . "' ORDER BY sort_order DESC");
		foreach ($query->rows as $result) { 
			$blog_data[$result['revblog_id']] = $this->getBlogInfo($result['revblog_id']);
		}
		return $blog_data;
	}
	
	public function getBlogRelated($revblog_id) {
		$blog_data = array();
		$query = $this->db->query("SELECT related_id FROM " . DB_PREFIX . "revblog_related WHERE revblog_id = '" . (int)$revblog_id . "'");
		if (isset($query->row['related_id']) && $query->row['related_id'] == 0) {
			$main_revblog_category = $this->getMainBlogCategory($revblog_id);
			$filter_data = array(
				'filter_category_id' => $main_revblog_category,
				'sort'  => 'bd.title',
				'order' => 'ASC',
				'start' => 0,
				'limit' => 9999
			);
			$blog_data = $this->getBlogs($filter_data);
		} else {
			foreach ($query->rows as $result) {
				$blog_data[$result['related_id']] = $this->getBlogInfo($result['related_id']);
			}
		}
		return $blog_data;
	}
	public function getBlogInfo($related_id) {
		$sql = "SELECT r.image, r.date_available, rd.revblog_id, rd.title, rd.description FROM " . DB_PREFIX . "revblog r LEFT JOIN " . DB_PREFIX . "revblog_description rd ON (r.revblog_id = rd.revblog_id) WHERE rd.language_id = " . (int)$this->config->get('config_language_id') . " AND rd.revblog_id = " .(int)$related_id;
		$query = $this->db->query($sql);
		$blog = $query->row;
		return $blog;
	}
	public function getMainBlogCategory($revblog_id) {
		$query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "revblog_to_category WHERE revblog_id = '" . (int)$revblog_id . "' AND main_category = '1'");
	...
	public function addBlogReview($revblog_id, $data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "revblog_review SET author = '" . $this->db->escape($data['name']) . "', revblog_id = '" . (int)$revblog_id . "', parent_review_id = '" . (int)$this->db->escape($data['parent_review_id']) . "', text = '" . $this->db->escape($data['text']) . "', date_added = NOW()");
		$review_id = $this->db->getLastId();
	}
	public function getReviewsByBlogId($revblog_id, $start = 0, $limit = 500) {
		$query = $this->db->query("SELECT rr.review_id, rr.parent_review_id, rr.author, rr.text, rr.text, rr.date_added FROM " . DB_PREFIX . "revblog_review rr WHERE rr.revblog_id = '" . (int)$revblog_id . "' AND rr.status = '1' AND rr.parent_review_id = '0' ORDER BY rr.date_added ASC LIMIT " . (int)$start . "," . (int)$limit);
		return $query->rows;
	}
	public function getTotalReviewsByBlogId($revblog_id, $start = 0, $limit = 500) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "revblog_review WHERE revblog_id = '" . (int)$revblog_id . "' AND status = '1'");
        return $query->row['total'];
	}
	public function getParentReviewsByBlogId($revblog_id, $review_id, $start = 0, $limit = 500) {
		$query = $this->db->query("SELECT rr.review_id, rr.parent_review_id, rr.author, rr.text, rr.text, rr.date_added FROM " . DB_PREFIX . "revblog_review rr WHERE rr.revblog_id = '" . (int)$revblog_id . "' AND rr.status = '1' AND rr.parent_review_id = '" . (int)$review_id . "' ORDER BY rr.date_added ASC LIMIT " . (int)$start . "," . (int)$limit);
		return $query->rows;
	}
	public function getBlogLayoutId($revblog_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "revblog_to_layout WHERE revblog_id = '" . (int)$revblog_id . "' AND store_id = '" . (int)$this->config->get('config_store_id') . "'");
		if ($query->num_rows) {
	...
	public function getBlogCategoryLayoutId($category_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "revblog_category_to_layout WHERE category_id = '" . (int)$category_id . "' AND store_id = '" . (int)$this->config->get('config_store_id') . "'");
		if ($query->num_rows) {
			return $query->row['layout_id'];
		} else {
			return 0;
	...
	public function getProducts($data = array()) {
        ...
        if (!$product_data) {
            $sql = "SELECT p.product_id, p.quantity > 0 as nalichie, p.price > 0 as nalichie2, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id)";
	...
	private function addwatermark($filename, $options) {
		...
		switch ($options['watermark_type']) {
			...
			default:
				...
				$font = DIR_TEMPLATE . 'revolution/stylesheet/fonts/Roboto-Regular.ttf';
	...
	public function getrevAttributes($product_id, $attribute_ids = 0)	{
		...
		$query = $this->db->query($sql);
		$razdelitel = $this->config->get('revtheme_filter')['razdelitel'];
		if ($razdelitel) {
			$attributes = array();
			foreach($query->rows as $row) {
				foreach(explode($razdelitel, $row['text']) as $text) {
					$attributes[] = array(
	...
	public function getrevOptions($product_id) {
		...
		foreach ($product_option_query->rows as $product_option) {
			...
			$product_option_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_option_value pov LEFT JOIN " . DB_PREFIX . "option_value ov ON (pov.option_value_id = ov.option_value_id) LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) WHERE pov.product_id = '" . (int)$product_id . "' AND pov.product_option_id = '" . (int)$product_option['product_option_id'] . "' AND pov.quantity > 0 AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY ov.sort_order");
	...
	public function getfilters($filter_data_filters) {
		$filters = array();
		$filters['fman'] = array();
		$products = $this->model_catalog_product->getProducts($filter_data_filters);
		foreach ($products as $product) {
			$v_id = 'fman' . $product['manufacturer_id'];
			if (isset($filters['fman']) && $product['manufacturer'] && !(isset($filters['fman'][$v_id]))) {
				$filters['fman'][$v_id] = array(
					'count' => 0,
					'name'  => $product['manufacturer']
				);
			}
			if (isset($filters['fman'][$v_id])) {
				++$filters['fman'][$v_id]['count'];
			}
			$options = $this->getrevOptions($product['product_id']);
			foreach ($options as $option) {
				$id = $option['option_id'];
				if (!(isset($filters['fo' . $id]))) {
					$filters['fo' . $id] = array();
				}
				foreach ($option['product_option_value'] as $val) {
					$v_id = 'fo' . $id . '-' . $val['option_value_id'];
					if (!(isset($filters['fo' . $id][$v_id]))) {
						$filters['fo' . $id][$v_id] = array(
							'count' => 0,
							'name'  => $val['name']
						);
					}
					if (isset($filters['fo' . $id][$v_id])) {
						++$filters['fo' . $id][$v_id]['count'];
					}
				}
			}
			$attributes = $this->getrevAttributes($product['product_id']);
			foreach ($attributes as $attribute) {
				$id = $attribute['attribute_id'];
				if (!(isset($filters['fa' . $id]))) {
					$filters['fa' . $id] = array();
				}
				$v_id = 'fa' . $id . '-' . crc32($attribute['text']);
				if (!(isset($filters['fa' . $id][$v_id]))) {
					$filters['fa' . $id][$v_id] = array(
						'count' => 0,
						'name'  => $attribute['text']
					);
				}
				if (isset($filters['fa' . $id][$v_id])) {
					++$filters['fa' . $id][$v_id]['count'];
				}
			}
		}
		return $filters;
	...
	public function initfilter($filter_data) {
		...
		$cache_filters = 'revfilters_all.' . crc32((int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . (int)$category_id);
		$filters = $this->cache->get($cache_filters);
		if (!$filters) {
			$filters = array();
			...
			$results_cache = $this->cache->get('revfilter.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . (int)$this->config->get('config_customer_group_id') . '.' . (int)$category_id);
			if (!$results_cache) {
				$results_cache = $results = $this->model_catalog_product->getProducts($filter_data);
				if ($this->config->get('revtheme_all_settings')['cache_on']) {
					$this->cache->set('revfilter.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . (int)$this->config->get('config_customer_group_id') . '.' . (int)$category_id, $results);
				}
			}
			foreach ($results_cache as $result) {
				...
				if (isset($setting['filter_option']) && (isset($setting['filter_options']) && $setting['filter_options'])) {
					...
					foreach ($options as $option) {
						if (isset($setting['filter_option'][$option['option_id']]) && ($setting['filter_option'][$option['option_id']]['option_type'] !== 'false')) {
							if (!isset($setting['filter_option'][$option['option_id']]['option_categories']) || (!in_array($category_id, $setting['filter_option'][$option['option_id']]['option_categories']))) {
								if (!(isset($filters['fo' . $option['option_id']]))) {
									$filters['fo' . $option['option_id']] = array(
										'id'     => 'fo' . $option['option_id'],
										'values' => array(),
										'title'  => $option['name'],
								...
								foreach ($option['product_option_value'] as $val) {
									if (!(isset( $filters['fo' . $option['option_id']]['values']['fo' . $option['option_id'] . '-' . $val['option_value_id']] ))) {
										$filters['fo' . $option['option_id']]['values']['fo' . $option['option_id'] . '-' . $val['option_value_id']] = array(
											'id'    => $filters['fo' . $option['option_id']]['id'] . '-' . $val['option_value_id'],
									...
									if (isset($filters['fo' . $option['option_id']]['values']['fo' . $option['option_id'] . '-' . $val['option_value_id']]) && $filter_count_products) {
										++$filters['fo' . $option['option_id']]['values']['fo' . $option['option_id'] . '-' . $val['option_value_id']]['count'];
									}
								}
							}
						}
				...
				if (isset($setting['filter_attribute']) && (isset($setting['filter_attributes']) && $setting['filter_attributes'])) {
					...
					foreach ($attributes as $attribute) {
						if (isset($setting['filter_attribute'][$attribute['attribute_id']]) && ($setting['filter_attribute'][$attribute['attribute_id']]['attribute_type'] !== 'false')) {
							if (!isset($setting['filter_attribute'][$attribute['attribute_id']]['attribute_categories']) || (!in_array($category_id, $setting['filter_attribute'][$attribute['attribute_id']]['attribute_categories']))) {
								...
								if (!(isset($filters['fa' . $attribute['attribute_id']]['values'][$filters['fa' . $attribute['attribute_id']]['id'] . '-' . crc32($attribute['text'])]))) {
									if ($attribute['text'] != '') {
										$filters['fa' . $attribute['attribute_id']]['values'][$filters['fa' . $attribute['attribute_id']]['id'] . '-' . crc32($attribute['text'])] = array(
								...
								if (isset($filters['fa' . $attribute['attribute_id']]['values'][$filters['fa' . $attribute['attribute_id']]['id'] . '-' . crc32($attribute['text'])]) && $filter_count_products) {
									++$filters['fa' . $attribute['attribute_id']]['values'][$filters['fa' . $attribute['attribute_id']]['id'] . '-' . crc32($attribute['text'])]['count'];
								}
							}
						}
					}
			...
			if ($this->config->get('revtheme_all_settings')['cache_on']) {
				$this->cache->set($cache_filters, $filters);
			}
		}
		$f_sort = array();
		$f_title = array();
		if (!empty($filters['filters'])) {
			foreach ($filters['filters'] as $fkey => $filter) {
				$filter_s = array();
				$filter_n = array();
				...
				array_multisort($filter_s, SORT_ASC, $filter_n, SORT_NATURAL, $filters['filters'][$fkey]['values']);
				$f_sort[$fkey] = $filter['sort'];
				$f_title[$fkey] = $filter['title'];
			}
			array_multisort($f_sort, SORT_ASC, $f_title, SORT_NATURAL, $filters['filters']);
		}
		return array($filters['filters'], $filters['price_min'], $filters['price_max']);
	}
	public function initfilter_gr($filter_data) {
		...
		foreach ($results_cache as $result) {
			...
			if (isset($setting['filter_option'])) {
				...
				$results_cache_opt_gr = $this->cache->get('revfilter_opt_gr.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . (int)$this->config->get('config_customer_group_id') . '.' . (int)$category_id);
				if (!$results_cache_opt_gr) {
					...
					if ($this->config->get('revtheme_all_settings')['cache_on']) {
						$this->cache->set('revfilter_opt_gr.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . (int)$this->config->get('config_customer_group_id') . '.' . (int)$category_id, $options);
					}
				}
			}
			$filter_a_grs = false;
			if (isset($setting['filter_a_grs'])) {
				...
				$results_cache_atr_gr = $this->cache->get('revfilter_atr_gr.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . (int)$this->config->get('config_customer_group_id') . '.' . (int)$category_id);
				if (!$results_cache_atr_gr) {
					...
					if ($this->config->get('revtheme_all_settings')['cache_on']) {
						$this->cache->set('revfilter_atr_gr.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . (int)$this->config->get('config_customer_group_id') . '.' . (int)$category_id, $attributes);
					}
				}
			}
		}
	...
	public function addanswer($product_id, $data) {
        ...
        if ($send_mail) {
			...
			if ($product_info) {
				$product_info_name = $product_info['name'];
				$product_info_url = $this->url->link('product/product', 'product_id=' . $product_id);
			} else {
				$product_info_name = '';
				$product_info_url = '';
			}
            ...
            $subject = sprintf($this->language->get('text_mail_subject_answer'), $this->config->get('config_name'));

            $message = $this->language->get('text_mail_waiting_answer') . " <a href='".$product_info_url."'>".$product_info_name."</a>" . "\n";
            $message .= sprintf($this->language->get('text_mail_reviewer_answer'), $this->db->escape(strip_tags($data['name_answer']))) . "\n";

            $message .= $this->language->get('text_mail_review_answer') . " ";
            $message .= $this->db->escape(strip_tags($data['text_answer'])) . "\n\n";
	...
	public function addProductNotify($data) {
		
		$db_query_notify = $this->db->query("SHOW TABLES LIKE '" . DB_PREFIX . "revproductnotify'");
		if (!$db_query_notify->num_rows) {
			$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "revproductnotify` (
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "revproductnotify SET product_id = '" . (int)$data['product_id'] . "', name = '" . $this->db->escape($data['firstname']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', email = '" . $this->db->escape($data['email']) . "', comment = '" . $this->db->escape($data['comment']) . "', product_name = '" . $this->db->escape($data['product_name']) . "', date_added = NOW()");
     	
		$this->language->load('revolution/revolution');

		$zagolovok = $this->language->get('text_mail_notify');
		$message = $this->language->get('text_compare_tovar') . ': '  .  $data['product_name'] . "\n";	
