	...
	public function index() {
		...
		if ($product_info) {
			...
			if ($product_info['quantity'] <= 0) {
				...
				$data['stock'] = $this->language->get('text_product_instock') . '<span class="hidden pr_quantity">' . $product_info['quantity'] . '</span>';
			}
			...
			$config_image_additional_width = $data['config_image_additional_width'] = $this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_width');
			$config_image_additional_height = $data['config_image_additional_height'] = $this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_height');
			...
			if ($settings_stikers['stock_status']) {
			  ...
			  if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
				...
				if ((float)$product_info['special'] && ($this->customer->isLogged() || !$this->config->get('config_customer_price'))) {
					...
					$this->load->model('revolution/revolution');
					$special = $this->model_revolution_revolution->getProductSpecialData($product_id);
				...
				foreach ($option['product_option_value'] as $option_value) {
				  if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
					...
					$product_option_value_data[] = array(
					  ...
					  'image'                   => $this->config->get('revtheme_product_all')['options_buy'] && !$this->config->get('revtheme_product_all')['option_img_options'] ? ($option_value['opt_image'] ? $this->model_tool_image->resize($option_value['opt_image'], 50, 50) : '') : ($option_value['image'] ? $this->model_tool_image->resize($option_value['image'], 50, 50) : ''),
					  'price'                   => $price,
