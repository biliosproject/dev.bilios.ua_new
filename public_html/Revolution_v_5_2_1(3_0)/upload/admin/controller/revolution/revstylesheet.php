...
if (isset($this->request->post['revtheme_catalog_stiker'])) {
	...
	$revtheme_catalog_stiker = $this->request->post['revtheme_catalog_stiker'];
	if (!$revtheme_catalog_stiker['status_mobile']) {
		$tcss .= '@media (max-width: 767px) {.product-thumb .image .stiker_panel, .product-thumb .fapanel {display: none;}}' . "\n";
	}
}
...
if (isset($this->request->post['revtheme_home_slideshow'])) {
	...
	if (isset($this->request->post['revtheme_home_slideshow']['allwide_padding'])) {
		$tcss .= '#top4, #top4 .col-sm-12.col-md-12, #top4 .owl-carousel .owl-wrapper-outer {padding: 0;}' . "\n";
		$tcss .= 'h1.home_h1 {padding-top: 20px;}' . "\n";
	}
}
if (isset($this->request->post['revtheme_all_settings'])) {
	...
	if (!$revtheme_all_settings['all_document_width']) {
		...
		$tcss .= '@media (min-width: 1360px) {#all_document {width: 1280px;}}' . "\n";
	}
	if ($revtheme_all_settings['all_content_width_wide']) {
		$tcss .= '@media (min-width: 1600px) {#all_document, .container {width: 1540px;} .box-col-3 {width: 1200.5%;} .box-col-2 {width: 810%;} .box-col-1 { width: 350%;}}' . "\n";
	}
	...
	if ($revtheme_all_settings['color_href']) {
		$tcss .= 'a,.list-group a,.pagination>li>a, .pagination>li>span, .mobile_info button:hover, .mobilemenu .navbar-nav > li > .list-group-submenu li a > .chevrond:hover, .mobilemenu .navbar-nav > li a > .chevrond:hover, .mobilemenu .navbar-nav > li.mobile_header_type_two > button > .chevrond:hover, .mobilemenu .navbar-nav > li.mobile_header_type_two > button > .chevrond {color: #' . $revtheme_all_settings['color_href'] . ';}' . "\n";
	...
	if ($revtheme_all_settings['cat_opis_opt']) {
		...
		$tcss .= '@media (min-width: 992px) {.product-grid .product-thumb:hover .description_options, .rev_slider .col-xs-12 .product-thumb:hover .description_options {display: block;} .product-grid .product-thumb.not_vivod_do:hover .description_options, .rev_slider .col-xs-12 .product-thumb.not_vivod_do:hover .description_options {display: none;}}' . "\n";
		$tcss .= '.rev_slider + .rev_slider {z-index: 4;}' . "\n";
		$tcss .= '.rev_slider + .rev_slider + .rev_slider {z-index: 3;}' . "\n";
		$tcss .= '.rev_slider + .rev_slider + .rev_slider + .rev_slider {z-index: 2;}' . "\n";
		$tcss .= '.rev_slider + .rev_slider + .rev_slider + .rev_slider + .rev_slider {z-index: 1;}' . "\n";
	}
	if ($revtheme_all_settings['cart_off']) {
		$tcss .= '#cart, .product-thumb .cart, .product-info .pop_left, .wwicons > a:first-child, .cart_wish .cart {display: none;}' . "\n";
	}
	if ($revtheme_all_settings['cat_compact']) {
		...
		$tcss .= '#content .row.products_category, #content .row.rev_slider_cat_compact {margin: 0 -4px 20px 0;}' . "\n";
	...
	//if ($revtheme_all_settings['mobil_two']) {
		//$tcss .= '@media (max-width: 767px) {.product-thumb .image .stiker_panel, .product-thumb .fapanel {display: none;}}' . "\n";
	//}
	if ($revtheme_all_settings['mobil_cat_opis']) {
		$tcss .= '@media (max-width: 991px) {.product-thumb .description_options {display: none;}}' . "\n";
	}
	if ($revtheme_all_settings['mobile_header']) {
		...
		if ($revtheme_all_settings['mobile_header'] == '2') {
			$tcss .= '@media (max-width: 767px) {#top2 .search {display: none;}}' . "\n";
		}
	}
	if (!$revtheme_all_settings['zatemnenie']) {
		$tcss .= '.zatemnenie_img {filter: none; opacity: 1;}' . "\n";
	}
}
...
if (isset($this->request->post['revtheme_product_all'])) {
	...
	if ($revtheme_product_all['desc_cols']) {
		$tcss .= '@media (min-width: 768px) {.tab-content #tab-description {-webkit-column-count: 2;-moz-column-count: 2;column-count: 2;-webkit-column-gap: 30px;-moz-column-gap: 30px;column-gap: 30px;-webkit-column-rule: 1px solid #eee;-moz-column-rule: 1px solid #eee;column-rule: 1px solid #eee;}}' . "\n";
	}
	if ($revtheme_product_all['spec_cols']) {
		$tcss .= '@media (min-width: 768px) {.tab-content #tab-specification, .tab-specification.rev_slider {-webkit-column-count: 2;-moz-column-count: 2;column-count: 2;-webkit-column-gap: 30px;-moz-column-gap: 30px;column-gap: 30px;-webkit-column-rule: 1px solid #eee;-moz-column-rule: 1px solid #eee;column-rule: 1px solid #eee;}}' . "\n";
		$tcss .= '@media (min-width: 768px) {.tab-content #tab-specification .table.attrbutes tbody tr td, .tab-specification.rev_slider .table.attrbutes tbody tr td {border: none;}}' . "\n";
	}
	if ($revtheme_product_all['options_two_in_line']) {
		$tcss .= '@media (min-width: 992px) {.options_no_buy {display: inline-block;width: 100%;margin-bottom: -10px;} .options_no_buy .form-group {width: 50%;float: left;padding-right: 15px;} .options_no_buy .form-group + .form-group {border-left: 1px solid #eee;padding-left: 15px;}}' . "\n";
	}
}
if (isset($this->request->post['revtheme_catalog_all'])) {
	...
	if (!$setting_catalog_all['ch_quantity']) {
		...
		$tcss .= '.product-layout.new_line .product-thumb .product_buttons .clearfix {display: none;}' . "\n";
	}
	...
	if ($setting_catalog_all['img_slider'] == '2') {
		$tcss .= '.owlproduct .owl-controls {display: none !important;}' . "\n";
	}
}
if (isset($this->request->post['revtheme_header_menu'])) {
	...
	if ($revtheme_header_menu['zadergka'] || $revtheme_header_menu['onclick']) {
		$tcss .= '.show-on-hover:hover > ul.dropdown-menu.dblock22 {display: block;}' . "\n";
	} else {
		$tcss .= '.show-on-hover:hover > ul.dropdown-menu, #menu2_button:hover #menu2, #top3.affix #menu2_button:hover #menu2 {display: block;}' . "\n";
	}
}
...
if (isset($this->request->post['revtheme_header_cart'])) {
	...
	$revtheme_header_menu = $this->request->post['revtheme_header_menu'];
	if ($revtheme_header_cart['type'] == 'standart' && $revtheme_header_menu['sticky']) {
		$tcss .= '#cart .dropdown-menu > li {max-height: 410px; overflow-y: auto;} #cart .dropdown-menu li > div {padding-top: 15px;}' . "\n";
	}
	if ($revtheme_header_cart['cart_position']) {
		$tcss .= '#top2 #cart {border-radius: 2px; margin-top: 10px;}' . "\n";
		$tcss .= '#top2 #cart button.cart {background-color: transparent; color: #' . $revtheme_all_settings['color_top3_cart'] . '; border: 1px solid #' . $revtheme_all_settings['color_top3_cart'] . '; border-radius: 2px;}' . "\n";
	}
}
if (isset($this->request->post['revtheme_catalog_popuporder'])) {
	$revtheme_catalog_popuporder = $this->request->post['revtheme_catalog_popuporder'];
	if ($revtheme_catalog_popuporder['quick_btn']) {
		$tcss .= '.product-thumb .zakaz {display: none !important;}' . "\n";
	}
}
